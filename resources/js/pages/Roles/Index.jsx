...
import { FaAvianex, FaEdit, FaTrash } from "react-icons/fa";
...
export default function RoleIndex({roles,permissions}) {
    const { abilities, checkAbility } = useAbilities('Role');
...
id:'',
    ...
    const columns = [
        { name: 'Role Name', selector: 'name', sortable: true, filterable: true, dataType: 'string' },
    ...
    roles.data.map((item, index) => {
            ...
            return {
                ...
                {abilities?.can_edit && <Link href={route("roles.edit",slug)} className='flex gap-4 items-center text-xl'><FaEdit></FaEdit> Edit</Link>}
                ...
                {abilities?.can_delete && <button className={`flex gap-4 items-center text-xl text-red-500`} onClick={() => setData(prev => ({...prev, showDeleteConfirm:true, id:slug}))}>
                    ...
                    {abilities?.can_assign &&
                       <button className={`flex gap-4 items-center text-xl`} onClick={() => setData(prev => ({...prev, showAssignPermission:true, id:slug, role:item}))}>
                        <FaAvianex />
                        Assign Permissions
                        </button>
                    }
     ...
     <AppLayout breadcrumbs={breadcrumbs}>
          ...
          <div className="p-6 text-slate-800  dark:text-slate-100 ">
            {abilities?.can_create && <div className="flex justify-between items-center mb-4">
                ...
                <Link href={route("roles.create")} className="bg-slate-500 text-white px-4 py-2 rounded-md hover:bg-slate-400 transition">
                    Create Role
                </Link>
            </div>}
        ...
        {data.showDeleteConfirm && <SweetAlert show={data.showDeleteConfirm}  confirm={true} message={`You about Deleting this Role. Are you sure you really want to delete this Role?`} action={() => destroy(route("roles.destroy",data.id))} cancel={() => setData(prev => ({...prev, showDeleteConfirm:false, id:null}))} />}
        {
            data.showAssignPermission && <AssignPermissionForm permissions={permissions} show={data.showAssignPermission} closeable={true} role={data.role} onClose={() => setData(prev => ({...prev, showAssignPermission:false, id:null}))} />
        }
      </section>
            </div>
     </AppLayout>
