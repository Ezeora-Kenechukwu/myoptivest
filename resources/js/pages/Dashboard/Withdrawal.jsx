...
const Withdrawal = ({errors, data, setData,processing }) => {
...
const handleWithdrawal = async () => {
  ...
  return (
    <div className='max-w-[511px] mx-auto'>
        <h1 className="font-rubik text-[#23272E] text-[32px] font-medium text-center ">Make a Withdrawal</h1>
        <p className='font-rubik font-normal text-[18px] text-[#23272E]  text-center'>Fill in the correct Withdrawal information</p>
          {/* Category */}
        <div className='my-2'>
 ...
 <div>
        ...
        {/* <div className=' my-5 '>
            ...
            <div className='flex flex-col gap-4 mt-5'>
                <label className={`flex items-center px-5 gap-6  h-[61px] rounded-[9px]   ${data.payment_method == 'paystack' ? 'border-[#4C44DB] border-2' : 'border-[#00000045] border-[0.3px] '}`} htmlFor='paystack'>
                <Checkbox checked={data.payment_method == 'paystack'} className='data-[state=checked]:bg-transparent data-[state=checked]:text-[#4C44DB] data-[state=checked]:border-[#4C44DB] checked:border-[#4C44DB] checked:text-[#4C44DB]'  id="paystack" value="paystack" name="payment_method" onClick={(e) =>  setData('payment_method', data.payment_method == "paystack" ? '': e.target.value)      } />
                    <img src='/PaystackLogo.png' className='' />
            </label>
        ...
        <button className='border border-[#5042DA] px-[24px] py-[16px] rounded-[100px] h-[56pxpx] w-full bg-[#5042DA] text-white text-[16px] font-rubik font-[600]' onClick={handleWithdrawal}>
                    Next
                </button>
    </div>
  )
