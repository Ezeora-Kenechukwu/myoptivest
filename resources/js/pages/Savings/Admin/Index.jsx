...
const Index = ({ breadcrumbs, auth, savings, plans, users }) => {
  ...
  const handleApprove = (savingId) => {
    post(route('admin.savings.approve', savingId), {
      preserveState: true,
      preserveScroll: true,
  ...
  const handleCancel = (savingId) => {
    post(route('admin.savings.cancel', savingId), {
      preserveState: true,
      preserveScroll: true,
      onSuccess: () => setSelectedSaving(null),
    });
    ...
    ? savings.data.map(item => ({
        ...
        action: (
          <DropdownComponent buttonText={<FaEllipsisVertical />}>
            ...
            {item.status !== 'started' && (
              <Link
                href={route('admin.savings.approve', item.id)}
                method="post"
                as="button"
                className="flex items-center justify-center gap-2 cursor-pointer w-full text-left px-2 py-1"
              >
            ...
            <Link
              href={route('admin.savings.cancel', item.id)}
              method="post"
              as="button"
              className="flex items-center justify-center gap-2 cursor-pointer w-full text-left px-2 py-1"
            >
